...
import './ProgramPartners.css'; // исправлен пробел в имени файла

const ProgramPartners = () => {
    // Партнеры с логотипами (используем .png)
    const partners = [
        {
            id: 1,
                            ...
                            <div className="partners-track">
                                {duplicatedPartners.map((partner, index) => (
                                    <img
                                        ...
                                        onError={(e) => {
                                            // Заменяем изображение на текстовый фолбэк
                                            e.target.style.display = 'none';
                                            const fallbackId = `fallback-${partner.id}-${index}`;
                                            // Проверяем, не создан ли уже фолбэк для этого элемента
